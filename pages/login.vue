                                ...
                                <v-form ref="loginForm" v-model="valid" lazy-validation @submit.prevent="userLogin">
                                    <v-row>
                                        ...
                                        <v-col class="d-flex" cols="12" sm="3" xsm="12" align-end>
                                            <v-btn x-large block :disabled="!valid" color="success" @click="validate" type="submit"> Login </v-btn>
                                        </v-col>
                                    </v-row>
                                </v-form>
                            </v-card-text>
  ...
  methods: {
    validate() {
      if (this.$refs.loginForm.validate()) {
          this.$auth.loginWith('local', {data:{ username: this.loginEmail ,password:this.loginPassword}})
          .then(() =>   this.$router.push('/') )
          .catch(error => {
            if (this.$axios.isCancel(error)) {
              console.log('Request canceled', error)
            } else {
              alert("here") //login failed
            }
          })
    }
    },
